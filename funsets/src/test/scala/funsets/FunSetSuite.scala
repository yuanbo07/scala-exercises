...
class FunSetSuite extends FunSuite {
  ...
  test("forall of set") {
    new TestSets {
      ...
      val p = (x:Int) => x != 2
      assert(!forall(a, p), "Forall")
    }
  }

  test("exist an element in a set") {
    new TestSets {
      val a = union(union(s1, s2), s3)
      val p1 = (x:Int) => x == 2
      val p2 = (x:Int) => x > 10
      assert(exists(a, p1), "Exist 1")
      assert(!exists(a, p2), "Exist 2")
    }
  }
  ...
  test("map a set") {
    new TestSets {
      ...
      val a = union(union(s1, s2), s3)
      val b = union(union(s2, s4), s6)
      assert(forall(b, (x:Int) => contains(map(a, (x:Int) => x * 2), x)), "Map")
    }
  }
}
