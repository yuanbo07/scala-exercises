...
object Anagrams {
  ...
  def combinations(occurrences: Occurrences): List[Occurrences] = occurrences match {
    ...
    case x :: xs => {
      x match {
        case (char, occ) =>
          for {
            list <- combinations(xs)
            occCombination <- 1 to occ
          } yield {
            (char, occCombination) :: list
          }
      }
    } ::: combinations(xs)
  }
  ...
  def subtract(x: Occurrences, y: Occurrences): Occurrences = {
    def updateMap(a: Map[Char, Int], b: (Char, Int)) = b match {
      case (b1, b2) => a.updated(b1, a(b1) - b2)
    }

    y.toMap.foldLeft(x.toMap)(updateMap).toList.filter{case (_, occ) => occ > 0}.sorted
  }
